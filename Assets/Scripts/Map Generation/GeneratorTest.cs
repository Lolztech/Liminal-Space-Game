...
public class GeneratorTest : MonoBehaviour
{
    ...
    [SerializeField] private List<SpecialRoomEntry> SpecialRooms = new List<SpecialRoomEntry>();

    [System.Serializable]
    public class SpecialRoomEntry
    {
        public string RoomName; // Name for developer reference
        public GameObject Prefab; // The prefab for this special room
        public Vector2Int RoomSize; // Dimensions of the room
        public int RoomCount; // Number of these rooms to generate
    }
    ...
    void PlaceSpecialRooms()
    {
        foreach (var specialRoom in SpecialRooms)
        {
            ...
            while (placedCount < specialRoom.RoomCount)
            {
                // Randomly select a location for the room
                Vector2Int location = new Vector2Int(
                    random.Next(0, size.x),
                    random.Next(0, size.y)
                );

                Room newRoom = new Room(location, specialRoom.RoomSize);
                Room buffer = new Room(location + new Vector2Int(-1, -1), specialRoom.RoomSize + new Vector2Int(2, 2));

                // Ensure the room does not intersect with others
                bool add = true;
                foreach (var room in rooms)
                {
                    if (Room.Intersect(room, buffer))
